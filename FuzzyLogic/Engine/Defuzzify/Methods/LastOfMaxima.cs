...
public class LastOfMaxima<T> : IDefuzzifier<T> where T : struct, IFuzzyNumber<T>
{
        ...
        InferenceMethod method = Mamdani)
    {
        ...
        return (function as IFuzzyInference)?.LambdaCutInterval(weight, method).GetValueOrDefault().X2;
    }
}